    ...
    ) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            logger.LogError(ex, "Erro ao injetar o SQL Scripts.");
            throw;
        }
    ...
    private static async Task EnsureDatabaseAsync(MasterNetDbContext dbContext, CancellationToken cancellationToken)
    {
    ...
    private static async Task RunMigrationAsync(MasterNetDbContext context, CancellationToken cancellationToken)
    {
    ...
    private static async Task SeedDataAsync(MasterNetDbContext context, CancellationToken cancellationToken)
    {
    ...
    private static async Task InitialSchemaSql(IConfiguration configuration, CancellationToken cancellationToken, ILogger<Worker> logger)
    {
        var connectionString = configuration.GetConnectionString("MasterNetDB");

        if (string.IsNullOrEmpty(connectionString))
        {
            throw new InvalidOperationException("Connection string 'MasterNetDB' not found.");
        }
        ...
        var sqlFilePath = Path.Combine(AppContext.BaseDirectory, "Scripts", "CreateDatabaseAndSeed.sql");
        if (!File.Exists(sqlFilePath))
            throw new FileNotFoundException($"SQL file not found: {sqlFilePath}");


        logger.LogInformation("O Arquivo SQL foi Encontrado {FilePath}", sqlFilePath);
        ...
        logger.LogInformation("Lendo os batchs: {BatchCount}", batches.Length);
        ...
        logger.LogInformation("SQL Scripts finalizados com sucesso");
    }
